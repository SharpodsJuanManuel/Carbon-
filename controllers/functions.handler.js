...
async function getAllCarbonMembershipMembers() {
  let members = [];
  let page = 1;
  let totalPages = 1;
  ...
  try {
    do {
      console.log(`Fetching carbon membership members page ${page}`);
      const response = await WooCommerce.getAsync(`memberships/members?plan=carbon&status=active&page=${page}&per_page=100`);
    ...
    console.error('Error fetching carbon membership members:', error);
  }
...
async function saveCarbonMemberEmails() {
  try {
    const members = await getAllCarbonMembershipMembers();

    console.log(`Total carbon memberships members fetched: ${members.length}`);
