...
const token = "7201900695:AAGS-ofV3DSY4a519AWL1xSEQsLcq3PR5dw";
    ...
    try {
      ...
      if (userEmail && userEmail.isActive) {
        ...
        await bot.sendMessage(chatId, 'Gracias por enviar tu correo. Aquí está tu invitación:', {
          reply_markup: {
            inline_keyboard: [
              [{ text: 'Unirse al Club Sharpods 💎', url: inviteLink.invite_link }]
            ]
          }
        });
  ...
  bot.on('message', async (msg) => {
    ...
    if (msg.chat.type === 'private') {
      if (!userStates[chatId] || userStates[chatId] === 'waiting_for_welcome') {
        ...
        await bot.sendMessage(chatId, `Bienvenido al ${channel.name}, espero que tengas suerte en tu inversión deportiva`);
